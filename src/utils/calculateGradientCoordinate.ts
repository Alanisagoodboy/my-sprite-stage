...
// export function calculateGradientCoordinate(width: number, height: number, angle = 180) {
...
export function calculateGradientCoordinate(anger: number) {
  
  
  return angleToCoordinates(anger);
}

export function parseGradientColor(color: string) {
  const gColor = gradient.parse(color)[0];
  return gColor;
}
